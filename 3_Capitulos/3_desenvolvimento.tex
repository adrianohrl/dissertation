\chapter[Desenvolvimento]{Desenvolvimento} \label{cap:desenvolvimento}
    ...
    \section{\textit{rqt\_mrta}}
        ...
        Esta interface gráfica utiliza a API em C++ do \textit{framework rqt} e pode ser utilizada como uma aplicação \textit{standalone} ou, então, como um \textit{plugin} que pode ser acoplado com outras ferramentas gráficas (vide Seção \ref{subsec:ros_gui}) do ROS em uma janela do \textit{rqt\_gui}.
        ...
        \subsection{Modelo}  \label{subset:model}
            ...
            %É através da classe \textit{System} que todas as demais classes do modelo são carregadas com as informações importadas dos arquivos de configuração de arquitetura e de aplicação. 
            
            
            %informação através do padrão de projeto cadeia de responsabilidade. Ou seja, a partir dos arquivos de configuração de arquitetura e de aplicação carregados através da camada de controle do padrão da arquitetura desse \textit{software}, são criadas novas instâncias de objetos do tipo \textit{Robot} e são encaminhadas para o objeto \textit{Problem}
